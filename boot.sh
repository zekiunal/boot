...
docker service create --replicas 1 --constraint 'node.role == manager' --constraint 'engine.labels.role == master' --with-registry-auth --limit-memory="128mb" -p 4000:4000 -e PORT=4000 --name="visualizer" -e SERVICE_8080_IGNORE="true" -e SERVICE_4000_NAME="visualizer" -e SERVICE_4000_TAGS="service" -e HOST=${HOST} --mount type=bind,source=/var/run/docker.sock,target=/var/run/docker.sock \ manomarks/visualizer
